...
if ($login_rank == 'Administrator') {
    ...
    /* Accepted Requests */
    $query = "SELECT COUNT(*)  FROM accepted_requests";
    $stmt = $mysqli->prepare($query);
    $stmt->execute();
    $stmt->bind_result($accepted_requests);
    $stmt->fetch();
    $stmt->close();
    ...
    while ($driv = $res->fetch_object()) {
        ...
        /* Driver Requests */
        $query = "SELECT COUNT(*)  FROM request WHERE request_status = 'Pending'";
        $stmt = $mysqli->prepare($query);
        $stmt->execute();
        $stmt->bind_result($request);
        $stmt->fetch();
        ...
        $query = "SELECT COUNT(*)  FROM accepted_requests WHERE accepted_request_driver_id = '{$driver_id}' ";
        $stmt = $mysqli->prepare($query);
        $stmt->execute();
        $stmt->bind_result($accepted_requests);
        $stmt->fetch();
    ...
    while ($cus = $res->fetch_object()) {
        ...
        /* Cars */
        $query = "SELECT COUNT(*)  FROM car WHERE car_customer_id = '{$customer_id}'";
        $stmt = $mysqli->prepare($query);
        $stmt->execute();
        $stmt->bind_result($car);
        $stmt->fetch();
